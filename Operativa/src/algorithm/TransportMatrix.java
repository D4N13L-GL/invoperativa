...
public class TransportMatrix {
	...
	public TransportMatrix(List<Factory> factories, List<PuntoDestino> destinations){
			...
			for (int j = 0; j < demands.length; j++) {
				if (this.matrix[i][j].isValid()) {
					if (val1 >= this.matrix[i][j].getCost() || val1 == -1){
						...
						if(val2 >= this.matrix[i][j].getCost() || val2 == -1){
	...
	private void buildMatrix() {		
		...
		for (int i = 0; i < factories.size(); i++) {
			for (int j = 0; j < destinations.size(); j++) {
				Costo newCost = costDAO.findCost(factories.get(i),destinations.get(j));
